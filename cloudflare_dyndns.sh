...
function ping_healthchecks {
  curl -sk -m 10 --retry 5 $healthchecks_url --data-raw "$msg"
}

# Get the current external IP address
current_ip=`curl -s -X GET https://ifconfig.co`
echo "Current IP is $current_ip"

record_ip=`dig +short A $dns_record @1.1.1.1`
